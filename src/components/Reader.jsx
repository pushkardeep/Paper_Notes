...
function ReadingWindow() {
  ...
  useGSAP(() => {
    gsap.from(readerRef.current, { left: "100%", duration: 0.4 });
  });
  ...
  const closeReadingWindow = () => {
    gsap.to(readerRef.current, {
      left: "100%",
      duration: 0.4,
      onComplete: () => dispatch(toogleRead()),
    });
  };
    ...
    >
      ...
      >
        ...
        <input
          ...
          onChange={(e) => dispatch(setEditNoteData({ title: e.target.value }))}
        />
        ...
        <textarea
          ...
          value={noteData.text}
          onChange={(e) => dispatch(setEditNoteData({ text: e.target.value }))}
        ></textarea>
      </form>
    </div>
  );
